...
async function main() {
  ...
  const categories = [
    {
      ...
      name: 'Bedspreads',
      description:
        'Handcrafted bedspreads in traditional and contemporary designs',
      image_url:
  ...
  const categoryBySlug = Object.fromEntries(
    categoryRows.map((c) => [c.slug, c]),
  );
  const materialByName = Object.fromEntries(
    materialRows.map((m) => [m.name, m]),
  );
    ...
    },
    {
      ...
      variants: [
        ...
        },
        {
          ...
          price: 99900,
          stock: 19,
          sku: 'TBL-ENS-COT-NVY-S4',
        },
      ],
    },
  ];

  // Seed data is an explicit development/test fixture. Keep it hidden until
  // all of its images and variants exist; never reactivate an existing row.
  ...
  for (const p of products) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const existingImages = await tx.productImage.count({
      ...
      for (const variant of variantRows) {
        const alreadyHasImage = await tx.productImage.count({
      ...
      if (!existingProduct) {
        await tx.product.update({
          where: { id: product.id },
          data: { is_active: true, published_at: seededPublishedAt },
        });
      }
    });
  }
