...
export function ImagesSection({ product }: { product: AdminProduct }) {
  ...
  async function handleFileChange(files: File[]) {
    ...
    for (const [index, sourceFile] of files.entries()) {
      try {
        const file = await preprocessImage(sourceFile);
        const result = await uploadAndAddImageAction(
          ...
          file,
          {
            ...
            is_primary:
              imageRole === "GALLERY" && !hasPrimaryGallery && index === 0,
        ...
        errors.push(
          error instanceof ImagePreprocessingError
            ? error.message
            : "We couldn't prepare that image. Choose a JPEG, PNG, or WebP under 3 MB and try again.",
        );
      }
    }
  ...
  async function handleMakeCover(imageId: string) {
    ...
    setError(null);
    const result = await setImageCoverAction(imageId, product.slug);
    setCoveringId(null);
    if (!result.ok) {
  ...
  return (
    ...
    >
      ...
      <FileUpload.Root
        ...
        onFileReject={() => setError("Choose a JPEG, PNG, or WebP image.")}
        mb="4"
      >
          ...
          {product.images.map((image) => (
            ...
            >
              ...
              <Box p="2">
                ...
                {image.image_role === "GALLERY" && !image.is_primary && (
                  <Button
                    size="xs"
                    variant="outline"
                    w="full"
                    mb="2"
                    loading={coveringId === image.id}
