...
function makeGalleryImages(): ProductImage[] {
  const galleryImages: ProductImage[] = variants.flatMap(
...
describe('ProductDetailInteractive', () => {
  ...
  it('keeps variant selection and uses the approved WhatsApp enquiry CTA in lead-generation mode', () => {
    render(
  ...
  it('filters material and color choices through the selected size hierarchy', () => {
    ...
    expect(
      screen.queryByRole('button', { name: 'Select color Yellow' }),
    ).not.toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: 'Queen' }));

    expect(screen.getByRole('button', { name: 'Queen' })).toHaveAttribute(
      'aria-pressed',
      'true',
    );
    expect(
      screen.queryByRole('button', { name: 'Select color Red' }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: 'Select color Blue' }),
    ).toHaveAttribute('aria-pressed', 'true');
    expect(
      screen.getByRole('button', { name: 'Select color Yellow' }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: 'Select color Green' }),
    ).not.toBeInTheDocument();
    expect(screen.getByTestId('product-gallery-main-image')).toHaveAttribute(
      'alt',
    ...
    expect(screen.getByRole('button', { name: 'Linen' })).toHaveAttribute(
      'aria-pressed',
      'true',
    );
    expect(
      screen.getByRole('button', { name: 'Select color Green' }),
    ).toHaveAttribute('aria-pressed', 'true');
    expect(
      screen.queryByRole('button', { name: 'Select color Blue' }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: 'Select color Yellow' }),
    ).not.toBeInTheDocument();
