...
## 1. Review the ready PRs — 15–30 minutes

1. Merge [PR #80](https://github.com/Gogulaanand/sunfabb-catalog/pull/80) — isolated Phase 6.10
   refund synchronization; all five checks pass.
...
2. Review [draft PR #79](https://github.com/Gogulaanand/sunfabb-catalog/pull/79), whose five checks
   pass, and mark it ready/merge it once the production variables are confirmed.
3. After deploy, run verification email, password reset, paid-order confirmation, and contact
...
## 3. Close remaining Phase 5 owner evidence — 45–90 minutes

1. Confirm Google Search Console ownership, submit/confirm the sitemap, and record ingestion state.
2. Observe one `whatsapp_click` and relevant contact event in GA4 Realtime/DebugView.
3. Paste a representative home, catalogue, and PDP URL into the actual social platforms and retain
   rendered-preview screenshots.
4. Run the priority journey once each on Android Chrome, iPhone Safari, and desktop Chrome. Record
   only observed results; do not substitute emulation for device evidence.

## 4. Complete Phase 4 WhatsApp Business — 2–4 hours
...
## 5. Start the long-lead GST and shipping inputs — 1–2 hours active time each
...
## 6. Final Phase 6.10 cutover — external lead time, then 2–4 hours active execution
