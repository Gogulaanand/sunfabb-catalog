...
Build correctly and quickly; let the code and commit messages speak for themselves.
Re-enable by removing this section.
...
Why: every message in a conversation re-sends the entire history.
By message 50, each response is reading ~30k tokens of context that aren't actively useful.
Long sessions also drift - they start as "build the Categories module" and end somewhere
unrelated.
...
Versioned in git.
Read by anyone cloning the repo.
Answers:
...
**Update only at phase boundaries or feature merges** - never every session.
Otherwise the commit history gets polluted with meta-state churn.

### `.session-state.md` - local, gitignored
Lives only on the active developer's machine.
Read at the start of a session, rewritten at the end.
Answers:
...
can pick up the project.
The session-level scratchpad changes every day and would just create commit noise.
Splitting keeps both files honest about their purpose.

### What if a fresh agent needs to pick up cold from another machine?
They get: `HANDOFF.md` (phase + branch) + the latest commit message + git log.
That's enough to know the project state at a phase boundary.
For mid-feature pickup, you'd write a focused session prompt anyway (see §7).
...
The orchestrator never silently builds large changes without review.
It delegates them to agents or batches them, then reviews output before committing.
...
Each item is tagged **L** (Learning track), **F** (Fast track), or **H** (Hybrid).

### Phases 0-5 - complete

Phases 0-5 are done and merged.
See the milestone log in `HANDOFF.md` for what was built in each phase.

### Phase 6 - e-commerce (remaining milestones)
...
Milestones 6.1-6.4 are done - see `HANDOFF.md` milestone table.

### Phase 7 - Reach & Growth

Detailed plan: `docs/GROWTH.md`.
Two streams run in parallel - Wave 0 is fully unblocked today.

| Item | Track | Notes |
|---|---|---|
| Wave 0: `robots.ts`, `sitemap.ts`, JSON-LD, OG/Twitter metadata, `noindex` on private pages, GA4 | F | Pure code/config; branch off `main`; no vendor accounts; works with 3 live products |
| Wave 0: ISR for `/catalog`, keep-alive fix | F | Performance; unblocked; existing web-vitals plan scopes it |
| Wave 1: trust pages (`/about`, `/contact`, policy pages) | L | Gated on owner business inputs (legal entity, GSTIN, return policy) |
| Wave 1: guides section (MDX), product/category copy batch | F | Content engine; depends on §3.1 shipping |
| Wave 1: AI/LLM GEO (robots allow-list, `llms.txt`, semantic HTML pass) | F | Unblocked once Wave 0 ships |
| Wave 1: Google Business Profile, Search Console/Bing | F | Owner creates accounts; Claude implements; depends on Wave 0 |
| Wave 2: Google Shopping feed, Meta catalog feed | F | Backend feed endpoints; gated on Phase 6.10 go-live |
| Wave 2: WhatsApp Business, ONDC listing | F | Owner-setup; gated on commerce go-live |
| Wave 2: Email retention (newsletter, post-purchase, abandoned-cart) | F | Depends on 6.7 (Resend) and 6.9 (cart sweep) |
| Wave 3: paid ads (Google PMax, Meta retargeting) | F | Gated on Wave 2 + GA4 conversion data |
...
5. If reviews pass: commit.
   If not: send specific fixes to that agent
...
Every spawned agent starts cold.
...
- Do not have one giant session that builds the whole backend.
  Break it.
- Do not spawn parallel agents on tasks with sequential dependencies.
  (e.g. don't build Products and Variants in parallel - Variants needs Products done first.)
- Do not update `HANDOFF.md` halfway through a session.
  Only at the end, reflecting what was actually completed.
- Do not let an agent run for >20 messages without reviewing its work.
...
Continuing to use a session past its scope.
If you finished what the session was for and find yourself thinking "while we're here, let me
also..." - stop.
Close the session.
The next bounded unit is a fresh session with a fresh prompt.
